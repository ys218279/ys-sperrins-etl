...
@pytest.fixture
def create_dim_design_object(s3_client, bucket_name_processed, object_key_dim_design, output_data_design):
    output_buffer = BytesIO()
    output_data_design.to_parquet(output_buffer)
    body = output_buffer.getvalue()
    s3_client.put_object(Bucket=bucket_name_processed, Key=object_key_dim_design, Body=body)

@patch("src.src_load.load_lambda.connect_to_dw")
def test_load_lambda_successfully_loads_tables(mock_conn_dw,
