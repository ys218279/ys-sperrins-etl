import pytest
import pandas as pd
from src.src_transform.transform_pandas import create_dim_design_table, create_dim_currency_table, create_dim_staff_table, create_dim_location_table, create_dim_counterparty_table, create_dim_date_table, create_fact_sales_order_table
from moto import mock_aws
import boto3
import logging

class TestDimDesign:
    def test_create_dim_design_creates_correct_schema(self,input_data_design, output_data_design):
...
class TestDimCurrency:
    def test_create_dim_currency_creates_correct_schema(self, input_data_currency, output_data_currency):
...
class TestDimStaff:
    def test_create_dim_staff_creates_correct_schema(self, input_data_staff, input_data_department, output_data_staff):
...
class TestDimLocation:
    def test_create_dim_location_creates_correct_schema(self, input_data_address, output_data_location):
...
class TestDimCounterparty:
    def test_create_dim_counterparty_creates_correct_schema(self, input_data_address, input_data_counterparty, output_data_counterparty):
    ...
    def test_create_dim_counterparty_table_key_error_logging(self, caplog):
        with mock_aws():
            ...
            with caplog.at_level(logging.ERROR):
                ...
                assert "Key Error, unable to create dim_counter_party table" in caplog.text
...
class TestFactSalesOrder:
    def test_create_fact_sales_order(self, input_data_sales_order,output_data_sales_order):
    ...
    def test_create_fact_sales_order_table_key_error_logging(self, caplog):
        with mock_aws():
            ...
            with caplog.at_level(logging.ERROR):
                ...
                assert "Key Error, unable to create fact_sales_order table" in caplog.text
