...
from transform_pandas import (create_dim_design_table, 
    ...
    try:
        ...
        for table, (transform_func, event_keys) in table_transform_map.items():
            ...
            if all(event.get(key) for key in event_keys):
                ...
                write_result = convert_df_to_s3_obj(s3_client, df_transformed, PROCESSED_BUCKET_NAME, obj_key_processed)
                if write_result:
        ...
        write_result = convert_df_to_s3_obj(s3_client, df_date, PROCESSED_BUCKET_NAME, obj_key_processed)
        if write_result:
